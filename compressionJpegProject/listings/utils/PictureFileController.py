...
class PictureFileController():
        ...
        for hauteur_image in range(0, hauteur, 8):
            for largeur_image in range(0, largeur, 8):
                ...
                for i in range(8):
                    ligne = []
                    for j in range(8):
                        if largeur_image + j < largeur and hauteur_image + i < hauteur:  # vérifie s'il reste suffisament de pixels pour ajouter dans le blocs
    ...
    def quantification(matrice,quantificationTable):
        return [[round(matrice[i][j]/quantificationTable[i][j]) for i in range(8)]for j in range(8)]
        
           
if __name__ == '__main__':
    owlBitmap = PictureFileController("jpeg_picture_100px.jpg")
    ...
    print(PictureFileController.quantification(matrice_apres_dct,PictureFileController.getQuantificationTable()))   
